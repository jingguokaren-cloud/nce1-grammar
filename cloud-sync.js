...
function mergeArrayMaps(localMap = {}, cloudMap = {}) {
  const merged = {};
  const keys = new Set([...Object.keys(localMap), ...Object.keys(cloudMap)]);

  keys.forEach((key) => {
    merged[key] = [...new Set([...(cloudMap[key] || []), ...(localMap[key] || [])])].sort();
  });

  return merged;
}

function mergeWork(localWork = {}, cloudWork = {}, preferLocal) {
  const merged = {};
  const keys = new Set([...Object.keys(localWork), ...Object.keys(cloudWork)]);

  keys.forEach((key) => {
    const localValue = localWork[key];
    const cloudValue = cloudWork[key];
...
function mergeVocab(localVocab = {}, cloudVocab = {}) {
  const merged = {};
  const words = new Set([...Object.keys(localVocab), ...Object.keys(cloudVocab)]);

  words.forEach((word) => {
    const localValue = localVocab[word];
    const cloudValue = cloudVocab[word];
    if (!localValue) merged[word] = cloudValue;
    else if (!cloudValue) merged[word] = localValue;
    else merged[word] = Number(localValue.ts || 0) >= Number(cloudValue.ts || 0)
      ? localValue
      : cloudValue;
  });
...
function mergeStates(localState = {}, cloudState = {}, cloudUpdatedAt = 0) {
  ...
  return {
    solved: mergeArrayMaps(localState.solved, cloudState.solved),
    work: mergeWork(localState.work, cloudState.work, preferLocal),
    stars: mergeArrayMaps(localState.stars, cloudState.stars),
    vocab: mergeVocab(localState.vocab, cloudState.vocab),
...
export async function createCloudSync({
  ...
  const elements = {
    ...
    loginToggle: document.getElementById("cloudLoginToggle"),
  ...
  function updateAuthUi() {
    ...
    setMessage(signedIn ? "" : "账号由家长创建；密码不会保存到网页中。");
    setStatus(
      signedIn
        ? `${currentDisplayName} · 云端记录已连接`
        : "登录后进入课程",
  ...
  async function findRecord() {
    const result = await collection.where({ appId: APP_ID }).limit(1).get();
  ...
  async function writeState(state) {
    ...
    if (documentId) {
      const result = await collection.doc(documentId).update(payload);
      if (result?.code) throw new Error(result.message || result.code);
      return;
    }
